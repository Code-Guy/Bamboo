...
namespace Bamboo
{
	...
	void DebugDrawManager::init()
	{
		...
		m_tick_timer_handle = g_runtime_context.timerManager()->addTimer(1.0f / UPDATE_BUFFER_FPS, [this]() { update(); }, true);
	}
	...
	void DebugDrawManager::destroy()
	{
		...
		g_runtime_context.timerManager()->removeTimer(m_tick_timer_handle);
	}
